...
void RenderStates::InitAll(ID3D11Device * device)
{
	...
	// 初始化深度/模板状态
	//
	D3D11_DEPTH_STENCIL_DESC dsleDesc;

	// 允许使用深度值一致的像素进行替换的深度/模板状态
	// 该状态用于绘制天空盒，因为深度值为1.0时默认无法通过深度测试
	dsleDesc.DepthEnable = true;
	dsleDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	dsleDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;

	dsleDesc.StencilEnable = false;

	HR(device->CreateDepthStencilState(&dsleDesc, DSSLessEqual.GetAddressOf()));
